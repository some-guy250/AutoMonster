...
class DebugTool(ctk.CTkFrame):
    def __init__(self, master, controller):
        ...
        capture_btn = ctk.CTkButton(
            button_frame,
            text="Capture New Assets",
            command=self.open_capture_tool,
            fg_color="#2ecc71",
            hover_color="#27ae60",
            height=32
        )
        capture_btn.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(5, 0))
    ...
    def load_assets(self):
        """Load all assets from utils.assets"""
        try:
            from utils import assets as _assets_module
            # Reload the module to get fresh data
            importlib.reload(_assets_module)

            self.all_assets = {}
            for attr in dir(_assets_module.ASSETS):
            ...
            self.filtered_assets = asset_list
            ...
            def insert_batch(start_idx=0):
                ...
                if end_idx < len(asset_list):
                    ...
                    self.after(5, lambda s=start_idx+batch_size: insert_batch(s))
                else:
    ...
    def refresh_from_disk(self):
        ...
        def do_refresh():
            try:
                ...
                # Remove assets module from sys.modules to force reload
                if 'utils.assets' in sys.modules:
                    del sys.modules['utils.assets']
        ...
        def insert_batch(start_idx=0):
            ...
            if end_idx < len(items_to_insert):
                ...
                try:
                    ...
                    logger.error(f"Failed to update scroll region: {e}", exc_info=True)
        ...
        try:
            ...
            for asset_name in self.selected_assets:
                ...
                try:
                    ...
                    logger.error(f"Error scanning {asset_name}: {e}")
    ...
    def _update_scan_results(self, matches_found):
        ...
        if self.current_matches:
            ...
            try:
                ...
                logger.error(f"Failed to log to GUI: {e}")
        else:
            ...
            try:
                ...
                logger.warning(f"Failed to log scan result: {e}")
                ...
                for name in dir(self.controller):
                    if not name.startswith('_'): # Skip private/protected
                        try:
                            ...
                            # Skip attributes that can't be accessed (properties, methods)
                            pass
