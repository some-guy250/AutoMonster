import time
import logging
from config.config import DEFAULT_DEVICE_WIDTH, DEFAULT_DEVICE_HEIGHT, SWIPE_START_Y_FRACTION
...
def debug_unlock():
    try:
        ...
        device = devices[0]
        logger.debug(f"Connected to device: {device.serial}")
        ...
        logger.debug(f"Screen on: {is_screen_on}")
        ...
        if not is_screen_on:
            ...
            dump_power = device.shell("dumpsys power")
            is_screen_on = "mWakefulness=Awake" in dump_power
            logger.debug(f"Screen on after wakeup: {is_screen_on}")
        
        # 2. Check if locked
        logger.debug("Checking lock state...")
        ...
        logger.debug(f"Device locked: {is_locked}")
        
        # Debug: print relevant parts of dump_policy
        logger.debug("Lock state details:")
        for line in dump_policy.splitlines():
            if "Keyguard" in line or "Showing" in line:
                logger.debug(f"  {line.strip()}")
        
        if is_locked:
            logger.debug("Device is locked. Attempting unlock methods...")
            
            # Method A: Menu Key (82)
            logger.debug("Method A: KEYCODE_MENU (82)")
            ...
            if not is_still_locked:
                logger.debug("Unlocked with Method A!")
                return

            # Method B: Direct Dismiss Command
            logger.debug("Method B: wm dismiss-keyguard")
            ...
            is_still_locked = ("mIsShowing=true" in dump_policy or "mKeyguardShowing=true" in dump_policy)
            if not is_still_locked:
                logger.debug("Unlocked with Method B!")
                return
            
            # Method C: Swipe Up
            logger.debug("Method C: Swipe Up")
            ...
            logger.debug(f"Swiping from ({center_x}, {start_y}) to ({center_x}, {end_y}) duration 300ms")
            ...
            is_still_locked = ("mIsShowing=true" in dump_policy or "mKeyguardShowing=true" in dump_policy)
            if not is_still_locked:
                logger.debug("Unlocked with Method C!")
                return

            # Method D: Longer Swipe
            logger.debug("Method D: Longer Swipe (1000ms)")
            ...
            is_still_locked = ("mIsShowing=true" in dump_policy or "mKeyguardShowing=true" in dump_policy)
            if not is_still_locked:
                logger.debug("Unlocked with Method D!")
            else:
                logger.error("Failed to unlock device with all methods.")
        else:
            logger.debug("Device is already unlocked.")
